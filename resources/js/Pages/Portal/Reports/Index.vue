...
<template>
  <PortalLayout title="Reports">
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">Export your company tickets and CSAT responses.</p>
    </div>
    <Card class="mt-4">
    ...
    <div class="mt-4 grid gap-4 md:grid-cols-2">
      <Card><CardContent class="flex items-center justify-between gap-3 p-4"><div><p class="font-medium">Tickets</p><p class="text-sm text-muted-foreground">CSV and PDF export</p></div><div class="flex gap-2"><Link :href="route('portal.reports.tickets.csv', params)"><Button variant="secondary"><Download /> CSV</Button></Link><Link :href="route('portal.reports.tickets.pdf', params)"><Button>PDF</Button></Link></div></CardContent></Card>
      <Card><CardContent class="flex items-center justify-between gap-3 p-4"><div><p class="font-medium">CSAT</p><p class="text-sm text-muted-foreground">Ratings and comments</p></div><div class="flex gap-2"><Link :href="route('portal.reports.csat.csv', params)"><Button variant="secondary"><Download /> CSV</Button></Link><Link :href="route('portal.reports.csat.pdf', params)"><Button>PDF</Button></Link></div></CardContent></Card>
    </div>
  </PortalLayout>
</template>
